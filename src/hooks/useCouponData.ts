...
export function useCouponData(options: UseCouponDataOptions = {}) {
  ...
  const fetchCoupons = useCallback(async (storeId: string, language: string = 'en') => {
    if (!storeId) {
      const error = ErrorFactory.validation.createStoreIdValidationError(storeId)
    ...
    try {
      ...
      if (!response.ok) {
        const error = ErrorFactory.api.createServerError(response.status, 'Failed to fetch menu data')
        handleError(error, 'fetchCoupons')
        onError?.(error.userMessage)
        return
      }
